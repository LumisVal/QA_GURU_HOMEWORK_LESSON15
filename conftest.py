...
def setup_browser(request):
    ...
    yield
    # Прикрепляем финальный скриншот, даже если тест успешен
    allure.attach(
...
def attach_video_after_test(request):
    ...
    # Ожидаем появления видео (более надёжно, чем time.sleep)
    base_url = remote_url.split("/wd/hub")[0]
    video_url = f"{base_url}/video/{video_name}"
    for _ in range(15):  # максимум 15 секунд ожидания
        try:
        ...
        allure.attach("Video file not found", name="video_error", attachment_type=allure.attachment_type.TEXT)
        return
